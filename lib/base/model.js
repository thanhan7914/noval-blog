...
class Model {
    ...
    static findOne(filter, ...oths) {
        let builder = this.getBuilder();
        if(_.isString(filter) && oths.length > 0)
            return builder.
                where(filter, oths[0]).
                first().getData();
    ...
    belongsTo(model, foreign_key, local_key) {
        ...
        return model.getBuilder()
            ...
            .where(tableName + '.' + this.constructor.__primaryKey, this.primaryValue)
            .first()
            .getData();
    }
...
class RSOneToMany {
    ...
    collection() {
        return this.target.getBuilder()
            ...
            .where(this.tableName + '.' + this.sourcePrimaryKey, this.source.primaryValue)
            .getData();
    }
